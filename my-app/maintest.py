...
nav_bar_color = ft.colors.GREEN_500
...
class CustomNavigationBar(ft.UserControl):
    def __init__(self, destinations, on_change):
        super().__init__()
        self.destinations = destinations[:3]  # Limit to 3 destinations
    ...
    def build(self):
        ...
        for dest in self.destinations:
            if 'badge' in dest:
                icon_content = ft.Badge(
                    content=ft.Icon(dest['icon'], size=18),  # Smaller icon
                    text=dest['badge']
                )
            else:
                icon_content = ft.Icon(dest['icon'], size=18)  # Smaller icon

            nav_destinations.append(
                ft.NavigationBarDestination(
                    icon_content=icon_content,
                    selected_icon=ft.Icon(dest['icon'], size=18),  # Smaller selected icon
                    label=dest['label']
                )
            )
        ...
        return ft.Container(
            ...
            border=ft.border.all(color=nav_bar_color, width=3),
            border_radius=10,
            padding=ft.padding.only(top=8),  # Add small padding to top and bottom
            expand=True,  # This will make the container expand to fill available space
        )

    def on_nav_change(self, e):
        index = e.control.selected_index
        dest = self.destinations[index]
        if self.on_change:
            self.on_change(dest['route'])
...
async def main(page: ft.Page):
    ...
    def route_change(route):
        ...
        nav_destinations = [
            {"icon": ft.icons.TABLE_VIEW_SHARP, "label": "Your Scorecards", "badge": "GL", "route": "/"},
            {"icon": ft.icons.ADD, "label": "Add", "route": "/add_scorecard"},
            {"icon": ft.icons.PERSON, "label": "User", "badge": "GL", "route": "/get_users"}
        ]
        nav_bar = CustomNavigationBar(nav_destinations, on_nav_change)
        ...
        if page.route == "/scorecard":
            ...
            page.views.append(
                ft.View(
                    "/scorecard",
                        [
                        ft.AppBar(title=ft.TextButton(score_data["course_name"], style=ft.ButtonStyle(color=primary_text_color), scale = 1.1,  icon="refresh", icon_color=primary_text_color, on_click=reload_data), color="BLACK", bgcolor=app_bar_color, toolbar_height=40, center_title = True),
            ...
            page.views.append(
                ft.View(
                    "/add_scorecard",
                        [
                        ft.AppBar(
                            title=ft.Text("Scorecard Group", style=ft.TextStyle(size=16,  color=primary_text_color, font_family="San Francisco")),
                            color=primary_text_color,
                            bgcolor=app_bar_color,
                            toolbar_height=40,
                            center_title = True,
                        ...
                        ft.Text("Select from the groups below\n that you administer", size=16, color=primary_text_color, font_family="San Francisco"),
                        ft.Container(
                            content = ft.ListView(
                                [
                                    ft.Column([
                                ...
                                expand=True
                            ) if len(groups) > 0 else ft.Text("Your profile is not the administrator for any groups. Create a new group to continue to create a new Scorecard.", size=16, color=primary_text_color, font_family="San Francisco"),
                            border=ft.border.all(2, ft.colors.PURPLE),
                                border_radius=10,
                                padding=10,
                                expand=True,
                        ...
                        ft.Text("or" if len(groups) > 0 else "", size=16, color=primary_text_color, font_family="San Francisco"),
                        ft.ElevatedButton(text="Create New Group", on_click=lambda _: page.go("/create_group")),
                    ],
                    bgcolor=default_bgcolor,
                    padding = 5,
                    scroll=ft.ScrollMode.AUTO
